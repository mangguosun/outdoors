...
;$(function(){
    ...
    $('.ajax-post').click(function(){
        var target,query,form,ls;
        var target_form = $(this).attr('target-form');
        var target_ls=$(this).attr('target-ls');
        var that = this;
        var nead_confirm=false;
        if( ($(this).attr('type')=='submit') || (target = $(this).attr('href')) || (target = $(this).attr('url')) ){
			target = $(this).attr('url');
            form = $('.'+target_form);
            if(target_ls === undefined ){ 
            	ls=false;
            }else{ 
            	ls  =$('.'+target_ls);
            }
    ...
    (function(){
        ...
        $(document).click(function() {
           // $(".dropdown").hide();
           // $(".btn-group-click .w_btn").find("i").removeClass("btn-arrowup");
        });
    })();
...
function home_alert(types){


		if(types==='hide'){ 

			$('.ajax_loading_msg').hide();
			...
			var jump = $('.ajax_loading_msg');
		 	// 得到距离上边框的距离
	 		var top = ($(window).height() - jump.height())/2;
		 	// 得到距离左边框的距离
			var left = ($(window).width() - jump.width())/2; 
			// 滚动条向下滚动的距离   
	    	var scrollTop = $(document).scrollTop();
			// 滚动条向右滚动的距离   
	    	var scrollLeft = $(document).scrollLeft();
	    	 //显示 框 并设置 
	    	jump.css('display','block').css('z-index',999);   
	    	jump.css( { position : 'absolute', 'top' : top +scrollTop,'left' : left + scrollLeft } ).show();
	    	$('.ajax_loading_mask').css({'width':$(window).width(),'height':$(window).height(),'display':'block'}).show();
