        ...
        if (msg.messages) {
           ...
           // paly_ios_sound();
            for (var index in msg.messages) {
				Notify("【" + msg['messages'][index]['title'] + "】"+msg['messages'][index]['ctime'] +msg['messages'][index]['content'], 'top-right', '1000', 'success', 'fa-check', true);
				var new_html = $('<li><a data-url="' + msg['messages'][index]['url'] + '" onClick="readMessage(this,' + msg['messages'][index]['id'] + ')"><div class="message"><span class="message-sender">' + msg['messages'][index]['title'] + '</span><span class="message-time">' + msg['messages'][index]['ctime'] +'</span><span class="message-subject"></span><span class="message-body"></span></div></a></li>');
				$(".dropdown-messages li.dropdown-footer").after(new_html);
            }
