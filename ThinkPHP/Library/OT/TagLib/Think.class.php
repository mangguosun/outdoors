...
class Think extends TagLib{
    ...
    protected $tags   =  array(
		...
		'officialnav' =>  array('attr' => 'field,name', 'close' => 1), //获取官方导航
    ...
    /* 导航列表 */
    public function _officialnav($tag, $content){
        $field  = empty($tag['field']) ? 'true' : $tag['field'];
        $tree   =   empty($tag['tree'])? false : true;
        $parse  = $parse   = '<?php ';
        $parse .= '$__NAV__ = M(\'Officialchannel\')->field('.$field.')->where("status=1")->order("sort")->select();';
        if($tree){
            $parse .= '$__NAV__ = list_to_tree($__NAV__, "id", "pid", "_");';
        }
        $parse .= '?><volist name="__NAV__" id="'. $tag['name'] .'">';
