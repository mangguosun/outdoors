...
class ArticleController extends BlogController
{
    ...
    public function lists($page = 1)
    {
        ...
        if ($children == '') {
			 ...
			 $map['status']=1;
			//获取当前分类下的文章
            $list = $Document->where($map)->page($page, $category['list_row'])->order('id desc')->select();
        ...
        /* 模板赋值并渲染模板 */
        $this->assign('category', $category);
		$this->assign('current', 'category_'.$category['id']);
    ...
    public function detail($id = 0, $p = 1)
    {
		...
		$get_qrcode = D('qrcode')->where(array('types'=>'blog','siteid'=>SITEID,'linkid'=>$id))->find();
		...
		$this->assign('current', 'category_'.$info['category_id']);
        $this->assign('category', $category);
        $this->assign('info', $info);
        $this->assign('page', $p); //页码
        $this->setTitle('{$info.title} —— 官方公告');
    ...
    private function category($id = 0)
    {
        ...
        $categoryModel =D('Blog/Category');
        /* 获取分类信息 */
        $category = $categoryModel->info($id);
