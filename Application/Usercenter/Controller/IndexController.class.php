...
class IndexController extends BaseController
{
    ...
    public function index($uid = null, $page = 1, $count = 10)
    {
		...
		if($info_type=="base_info"){
            //调用API获取基本信息
            //TODO tox 获取省市区数据
            //$user = query_user(array('nickname', 'signature', 'email', 'mobile', 'rank_link', 'sex', 'pos_province', 'pos_city', 'pos_district', 'pos_community','real_name','qq','self_introduction'), $uid);
            $user = query_user(array('nickname','id', 'signature', 'email', 'mobile', 'rank_link', 'sex','address','real_name','qq','self_introduction','constellation'), $uid);
		...
		if(is_login() !=$user['id']){
		   //--浏览当前页面--id--关注了我--
			$who_follow=D('follow')->where("follow_who=".is_login()." and who_follow=".$user['id'])->find();
			$follow_who=D('follow')->where("follow_who=".$user['id']." and who_follow=".is_login())->find();
			if($who_follow>0 && $follow_who>0){
			  $this->assign('user', $user);
			}else{
			   $user['real_name']='';
			   $user['mobile']='';
    ...
    {
        $user_info = query_user(array('avatar128','nickname','uid', 'is_use','checked', 'space_url', 'icons_html', 'score', 'title', 'fans', 'following', 'weibocount', 'rank_link', 'signature'), $uid);
    ...
    public function information($uid = null, $info_type ="base_info")
    {
        ...
        if($info_type=="base_info"){
            //调用API获取基本信息
            //TODO tox 获取省市区数据
            //$user = query_user(array('nickname', 'signature', 'email', 'mobile', 'rank_link', 'sex', 'pos_province', 'pos_city', 'pos_district', 'pos_community','telphone','real_name','qq','self_introduction'), $uid);
            $user = query_user(array('nickname', 'signature', 'email', 'mobile', 'rank_link', 'sex','address','real_name','qq','self_introduction','constellation'), $uid);
            if($user['pos_province']!=0){
                $user['pos_province']=D('district')->where(array('id'=>$user['pos_province']))->getField('name');
                $user['pos_city']=D('district')->where(array('id'=>$user['pos_city']))->getField('name');
                $user['pos_district']=D('district')->where(array('id'=>$user['pos_district']))->getField('name');
