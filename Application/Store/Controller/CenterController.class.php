...
class CenterController extends StoreController
{
    ...
    public function createShop()
    {
        ...
        if (IS_POST) {
            ...
            $shop = $shopModel->where(array('uid' => is_login()))->find();
    ...
    public function post()
    {
        ...
        if ($aInfoId != 0) {
            ...
            if (($goods['uid'] != is_login() && !is_administrator()) || (!$goods)) {
                $this->error('404不存在。');
            }
        ...
        $my_shop = D('Shop')->getShopByUid(is_login());
            ...
            if ($rs_info && $rs_data) {
                ...
                if ($entity['need_active']) {
                    ...
                    if ($entity['show_nav']) {
                        ...
                        $weiboApi->sendWeibo("我上架了一个新的 " . $entity['alias'] . " 【" . $info['title'] . "】：" . $postUrl);
                    }
    ...
    {
        $shop = D("Shop")->where(array('uid' => is_login()))->find();
        if (!$shop) {
