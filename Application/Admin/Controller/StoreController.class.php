...
class StoreController extends AdminController
{
    ...
    function config()
    {
        ...
        if (!$data) {
            ...
            $data['CURRENCY_TABLE'] = 'store_currency';
            $data['CURRENCY_FIELD'] = 'currency';
        ...
        $configBuilder->title('微店配置')
            ...
            ->keyText('CURRENCY_TABLE', '货币所在表', '默认为微店专用表 store_currency')
            ->keyText('CURRENCY_FIELD', '货币所在字段', '默认为微店专用字段 currency')
    ...
    public function shop()
    {
        ...
        $listBuilder->title('商品管理')->keyId()->keyImage('logo', '店铺图标')->keyText('shop_name', '店铺名')->keyUid()
    ...
    public function order($r = 15)
    {
        ...
        $listBuilder->title('商品管理')->keyId()->keyUid()
            ->keyText('r_name', '收货人名')
            ->keyText('amount', '订单总价')
