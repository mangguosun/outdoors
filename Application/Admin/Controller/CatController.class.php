...
class CatController extends AdminController
{
    ...
    public function info($entity_id = 0, $page = 1, $r = 20, $title = '')
    {
        ...
        if ($entity_id != 0) {
            ...
            $d = D('cat_info')->where($map)->order('create_time desc')->page($page, $r)->select();
            $total = D('cat_info')->where($map)->count();
     ...
     * 删除实体
     */
    function doDelEntity($ids, $status = -1)
    {
    ...
    public function editEntity($entity_id = 0)
    {
        ...
        if (IS_POST) {
            ...
            $entitys = D('cat_entity')->limit(999)->select();
            ...
            $configBuilder
                ...
                ->keyBool('can_rec', '允许接收其他模型信息', '在详情页显示发送按钮，用户可以向发布者发送其他模型的信息')
