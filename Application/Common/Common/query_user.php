...
function query_user($fields = null, $uid = null)
{
    ...
    if (in_array('mobile_space_url', $fields)) {
        $result['mobile_space_url'] = U('Mobile/People/peopledetail', array('uid' => $uid));
    }
...
function read_people_info_cash(){ 
    return S('peoples_index_member_'.SITEID);

}
function write_people_info_cash($value){ 
    return S('peoples_index_member_'.SITEID,$value,3600);
}

function clean_people_info_cash(){ 
    S('peoples_index_member_'.SITEID, null);
}