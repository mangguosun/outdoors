 ...
 * @auth 陈一枭
 */
function getThumbImage($filename, $width = 100, $height = 'auto', $type = 99, $replace = false)
{
    $UPLOAD_URL = '';
    $UPLOAD_PATH = '';
    ...
    if (!file_exists($UPLOAD_PATH . $oldFile)) {
        ...
        if ($old_image_width <= $width && $old_image_height <= $height) {
        	if($type==99){ 
            	...
            	$image->open($UPLOAD_PATH . $oldFile);
            	$image->thumb($width,$height,\Think\Image::IMAGE_THUMB_CENTER)->save($UPLOAD_PATH . $thumbFile);
