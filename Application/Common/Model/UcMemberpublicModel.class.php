...
class UcMemberpublicModel extends Model{
    ...
    protected $_validate = array(
		...
		array('email', 'checkEmail', -8, self::EXISTS_VALIDATE, 'callback'), //邮箱被占用
	...
	/*验证密码*/
	protected function checkNickpassword($password){
	      preg_match('/^[a-zA-Z][a-zA-Z0-9]{6,15}$/', $password, $result);
		 if (!$result) {
	    return false;
	  }
	    return true;
    ...
    */
    ...
    public function quickLogin($username){
    	...
    	if(is_array($user)){ 
    		...
    		if(preg_match('/^1[0-9]{10}$/',$username)){ 
    			$result=$this->register('',$nickname2,$password,'',$mobile);
    			if($result>0){ 
    ...
    public function register($username, $nickname, $password, $email, $mobile){
        ...
        if ($usercenter_member) {
            $result=D('Memberpublic')->registerMember($nickname);
            if($result>0){
                $usercenter_member['id']=$result;
				$usercenter_member['siteid']=SITEID;
                $uid = $this->add($usercenter_member);

                D('Common/Dynamic')->sendMessage($uid,'Register','','' ,'');
     ...
     */
    public function login($username, $password, $type = 2,$siteid=SITEID)
    {
        $map = array();
        switch ($type) {
            case 1:
