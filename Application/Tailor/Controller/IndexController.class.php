...
class IndexController extends Controller
{
	...
	public function doPost($minprice='',$maxprice='',$totalnum='',$childnum='',$minage='',$maxage='',$desty='',$earlytime='',$latetime='',$traveldays='',
	$tag='',$other='',$unsure='',$reference = '',$desty_type='',$contact_name='',$contact_telephone='',$contact_email=''){
		...
		if($rs){
			...
			*/
			...
			$this->success('提交成功！',U('Tailor/Index/tailor_info',array('id'=>$rs)));
		}else{
			$this->error('提交失败！');
		}
	}
	...
	public function checkEmail($email)
    {
     ...
     public function checkUrl($url){
        $url = trim(op_h($url));
		$pa = "/^(https?:\/\/)?(((www\.)?[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)?\.([a-zA-Z]+))|(([0-1]?[0-9]?[0-9]|2[0-5][0-5])\.([0-1]?[0-9]?[0-9]|2[0-5][0-5])\.([0-1]?[0-9]?[0-9]|2[0-5][0-5])\.([0-1]?[0-9]?[0-9]|2[0-5][0-5]))(\:\d{0,4})?)(\/[\w- .\/?%&=]*)?$/i";
		preg_match($pa,$url,$r);
		if(empty($r)){
