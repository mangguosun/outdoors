...
class IndexController extends BaseController
{
    ...
    public function post()
    {
        ...
        if ($info_id != 0) {
            ...
            if (($info['uid'] != is_login() && !is_administrator()) || (!$info)) {
                $this->error('404不存在。');
            }
        ...
        if ($rs_info != 0) //如果info保存成功
        {
            ...
            if ($rs_info && $rs_data) {
                ...
                if ($entity['need_active']) {
                    ...
                    if ($entity['show_nav']) {
                        $postUrl = U('cat/index/info', array('info_id' => $rs_info),null,true);
                        $weiboApi = new WeiboApi();
                        $weiboApi->resetLastSendTime();
                        $weiboApi->sendWeibo("我发布了一个新的 ".$entity['alias'] ."信息 【" . $info['title'] . "】：" . $postUrl);
    ...
    public function delInfo()
    {
        ...
        $rs = D('cat_info')->where(array('id' => I('post.info_id', 0, 'intval'), 'uid' => is_login()))->delete();
        if ($rs) {
    ...
    public function doScore()
    {
        ...
        if ($rs) {
            $map_info['info_id'] = I('post.info_id', 0, 'intval');
