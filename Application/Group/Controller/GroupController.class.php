...
class GroupController extends Controller
{
    public function _initialize()
    {
		...
		$menutype[$rs['id']]['icon'] = $model_info['icon'];
		$groupType = D('GroupType')->where(array('status' => 1,'pid'=>0,'siteid'=>SITEID))->select();
		if($groupType){
			foreach($groupType as $m=> &$rs){
				$menutype[$rs['id']]['tab'] = 'category_'.$rs['id'];
				$menutype[$rs['id']]['title'] = $rs['title'];
				$menutype[$rs['id']]['href'] =  U('groups',array('cate'=>$rs['id']));
				...
				if($childgroupType){
					foreach($childgroupType as $mc=> &$rsc){
						...
						$menutype[$rs['id']]['children'][$rsc['id']]['href'] = U('groups',array('cate'=>$rsc['id']));
					}
				}
			}
		}
        ...
        $sub_menu =
            array(
				...
				'btnhelf' =>
                    array(
                        array('title' => '创建群组', 'href' =>is_login()?U('group/index/create'):"javascript:toast.error('登录后才能操作')", 'icon' => 'edit'),
                    )
            );
    ...
    public function getGroup($id)
    {
        ...
        $group = S('group_' . $id);
        if (empty($group)) {
            $group = D('Group/Group')->where(array('status' => 1, 'id' => $id,'siteid'=>SITEID))->find();
            $group['post_list'] = D('Group/GroupPost')->where(array('status' => 1, 'group_id' => $group['id'],'siteid'=>SITEID))->limit(4)->select();
            $group['member_count'] = D('GroupMember')->where(array('group_id' => $group['id'], 'status' => 1,'siteid'=>SITEID))->count();
            $group['month_count'] = D('GroupPost')->where(array('create_time' => array('between', array(strtotime("-1 month"), time())), 'group_id' => $group['id'],'siteid'=>SITEID))->count();
            $group['user'] = query_user(array('avatar128', 'avatar64', 'nickname', 'uid', 'space_url', 'icons_html'), $group['uid']);
            $group['user']['group_count'] = D('Group')->where(array('uid' => $group['uid'], 'status' => 1,'siteid'=>SITEID))->count();
    ...
    {
        $group = D('Group')->where(array('id' => $group_id, 'status' => 1,'siteid'=>SITEID))->find();
        return $group['type'];
    }
    ...
    {
        $type = D('GroupType')->where(array('id' => $type_id, 'status' => 1,'siteid'=>SITEID))->find();
        return $type['title'];
    }
    ...
    {
        $type = D('GroupPostCategory')->where(array('id' => $type_id, 'status' => 1,'siteid'=>SITEID))->find();
        return $type['title'];
    }
    ...
    {
        if (!$this->isLogin()) {
            //$this->error('需要登录才能操作',U('Home/User/login'));
    ...
    protected  function requireAllowEditPost($post_id)
    {
        ...
        //确认帖子时自己的
        $post = D('GroupPost')->where(array('id' => $post_id, 'status' => 1,'siteid'=>SITEID))->find();
    ...
    {
        $post = D('GroupPost')->where(array('id' => $post_id,'siteid'=>SITEID))->find();
        if (!$post) {
            $this->error('帖子不存在');
        }
    }
    ...
    protected function isGroupAllowCurrentUserGroup($group_id)
    {
        ...
        //读取贴吧的基本信息
        $group = D('Group')->where(array('id' => $group_id,'siteid'=>SITEID))->find();
        $userGroups = explode(',', $group['allow_user_group']);

        //读取用户所在的用户组
        $list = M('AuthGroupAccess')->where(array('uid' => is_login(),'siteid'=>SITEID))->select();
        foreach ($list as &$e) {
            $e = $e['group_id'];
        }
    ...
    protected function getGroupTypes()
    {
        ...
        foreach($groupType as $v){
            $child[$v['id']] = D('GroupType')->where(array('status' => 1,'pid'=>$v['id'],'siteid'=>SITEID))->select();
        }
        $this->assign('childType', $child);

        foreach ($groupType as $k => $v) {
            $child = D('GroupType')->where(array('pid' => $v['id'], 'status' => 1,'siteid'=>SITEID))->order('sort asc')->select();
            //获取数组中第一父级的位置
    ...
    {
        $notice = D('GroupNotice')->where('group_id=' . $group_id.' and siteid ='.SITEID)->find();
    ...
    {
        $post = D('GroupPost')->where('id=' . $post_id.' and siteid ='.SITEID)->find();
