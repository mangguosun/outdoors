...
class AdminListBuilder extends AdminBuilder
{
    ...
    public function sub_title($sub_title)
    {
    ...
    {
        return $this->button('清空', array('class' => 'w_btn ajax-post tox-confirm', 'data-confirm' => '您确实要清空回收站吗？（清空后不可恢复）', 'url' => U('', array('model' => $model)), 'target-form' => 'ids'));
    }
    ...
    public function keyLink($name, $title, $getUrl)
    {
	 ...
	 public function keyCommentLink($name, $title, $getUrl)
    {
        //如果getUrl是一个字符串，则表示getUrl是一个U函数解析的字符串
        if (is_string($getUrl)) {
            $getUrl = $this->createCommentGetUrlFunction($getUrl);
        }
    ...
    public function keyNewLink($name,$title,$getUrl){
      
	   //如果getUrl是一个字符串，则表示getUrl是一个U函数解析的字符串
        if (is_string($getUrl)) {
            $getUrl = $this->createNewGetUrlFunction($getUrl);
        }
    ...
    public function keyStatus($name = 'status', $title = '状态')
    {
	 ...
	 public function keyDoStatus($name = 'status', $title = '')
    {
        $map = array(-1 => '删除', 0 => '启用', 1 => '禁用', 2 => '未审核');
        return $this->key($name, $title, 'status', $map);
    }
	
	  //返原
	public function KeyStatusReversion($name = 'status', $title = '操作'){
		 $map = array(-1 => '还原', 0 => '禁用', 1 => '启用', 2 => '未审核');
        return $this->key($name, $title, 'status', $map);
		
	}
	
	 public function keyVideoStatus($name = 'status', $title = '状态')
    {
        $map = array(-1 => '删除', 0 => '禁用', 1 => '启用', 2 => '未审核');
        return $this->key($name, $title, 'status', $map);
    }
	...
	//wuqingjing
	 public function keyPointDoAction($getUrl, $text, $title = '操作')
    {
        //获取默认getUrl函数
        if (is_string($getUrl)) {
            $getUrl = $this->createPointGetUrlFunction($getUrl);
        }
    ...
    //wuqingjing
	 public function keyPointActionEdit($getUrl, $text = '编辑')
    {
        return $this->keyPointDoAction($getUrl, $text);
    }
    ...
    public function display()
    {
        ...
        $this->convertKey('uid', 'text', function ($value) {
        	if($value==0){ 
        		$value1['nickname']="游客";
        		return $value1['nickname'];
        	}else{ 
        		$value = query_user(array('username', 'uid', 'space_url', 'nickname'), $value);
        ...
        $this->convertKey('link', 'html', function ($value, $key, $item) {
			...
			if($url){
			   ...
			   return "<a href=\"$url\" target='_blank' class='glyphicon glyphicon-link' title=$value>".getShortSp(op_h($value, 'text'), 10)."</a>";
			}else{
			   return "<span title='你还没有添加链接地址'>$value</span>";
		    }
        });
		...
		$this->assign('sub_title', $this->_sub_title);
    ...
    public function doSetStatus($model, $ids, $status)
    {
    	...
    	//wuqingjing
	public function doPointSetStatus($model, $ids, $status)
    {	
        $ids = is_array($ids) ? $ids : explode(',', $ids);
        M($model)->where(array('ptypeid' => array('in', $ids)))->save(array('status' => $status));
        $this->success('设置成功', $_SERVER['HTTP_REFERER']);
    }
    
	/**new*/
	 public function doSetNewStatus($model, $ids, $status)
    {
        $ids = is_array($ids) ? $ids : explode(',', $ids);
        M($model)->where(array('id' => array('in', $ids)))->save(array('status' => $status,'display'=>0));
        $this->success('设置成功', $_SERVER['HTTP_REFERER']);
    }
	
	 public function doSetVideoStatus($model, $ids, $status)
    {
        $ids = is_array($ids) ? $ids : explode(',', $ids);
        M($model)->where(array('id' => array('in', $ids)))->save(array('status' => $status));
        $this->success('设置成功', $_SERVER['HTTP_REFERER']);
	...
	//wuqingjing
	private function createPointGetUrlFunction($pattern)
    {
        return function ($item) use ($pattern) {
            $pattern = str_replace('###', $item['cardid'], $pattern);
            //调用ThinkPHP中的解析引擎解析变量
            $view = new \Think\View();
            $view->assign($item);
            $pattern = $view->fetch('', $pattern);
	...
	*评论
	*/
	 private function createCommentGetUrlFunction($pattern)
    {
	
        return function ($item) use ($pattern) {
            $pattern = str_replace('###', $item['row_id'], $pattern);
            //调用ThinkPHP中的解析引擎解析变量
            $view = new \Think\View();
            $view->assign($item);
            $pattern = $view->fetch('', $pattern);
	 ...
	 private function createNewGetUrlFunction($pattern)
    {
	
        return function ($item) use ($pattern) {
			$urlArray=explode("=",$pattern,2);
			$pattern = str_replace('###', $item[$urlArray[0]], $pattern);
			$myArray=explode("=",$pattern,2);
		    $pattern=$myArray[1];
			if(!empty($pattern)){
				return $pattern;
			}else{
    ...
    public function clearTrash($model = '')
    {
        if (IS_POST) {
            if ($model != '') {
                $result = D($model)->where(array('status' => -1,'siteid' => SITEID))->delete();
                if ($result) {
