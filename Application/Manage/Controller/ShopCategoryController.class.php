...
class ShopCategoryController extends BaseController
{
	...
	**/
	public function shop_category($tab=0){
		...
		if($tab==0){
			$tree = $this->shop_categoryModel ->getTree();
     ...
     */
    public function add($title='',$sort=0,$pid=0,$level=0,$category_pic='',$description=''){
        if(IS_POST){
			...
			$info['pid'] = I('pid');
			$info['page_title'] = '新增商品分类';
            $this->assign('info',$info);
			$category = D('ShopCategory')->where(array('siteid'=>SITEID,'pid'=>0,'status'=>array('gt',0)))->field(true)->select();
	 ...
	 * @author yangweijie <yangweijiester@gmail.com>
	 */
    public function edit($id = 0,$title='',$sort=0,$pid=0,$level=0,$category_pic='',$description=''){
        if(IS_POST){
            $Category = D('ShopCategory');
			$title=trim($title);
		    if($title=='')$this->error('请输入分类名称');
