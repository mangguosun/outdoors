...
class PaydepositModel extends Model{
	...
	public function set_record($data){
		$require_items = array('uid','siteid','username','email','contactname','telephone','trade_sn','money','quantity','addtime','paytime','usernote','usernote','pay_type','pay_id','payment','status','order_type','type','order_id','order_paytype','website_url');
		if(is_array($data)) {
	...
	public function get_paytype() {
		$result = $this->pay_db->where(array('enabled'=>array('in','1,3'),'pay_platform'=>'mobile'))->select();
		$weixin=strpos($_SERVER['HTTP_USER_AGENT'],"MicroMessenger");
		$info = array();
		foreach($result as $r) {
			if(!$weixin && $r['id']==5) continue;
