...
SQL
    ...
    public function localCommentMobile($param)
    {
        $path = $param['path'];
        $uid = $param['uid'];
        $title = $param['title'];
        ...
        //增加用户信息
        foreach ($list as &$e) {
            $e['user'] = query_user(array('uid', 'avatar64', 'nickname', 'mobile_space_url'),$e['uid']);
        }
        unset($e);
        //显示页面
        $this->assign('title',$title);
        $this->assign('cover_id',$cover_id);
        $this->assign('list', $list);
        $this->assign('total_count', $total_count);
        $this->assign('count', $count);
        $this->assign('app', $app);
        $this->assign('mods', $mod);
        $this->assign('row_id', $row_id);
        $this->assign('uid', $uid);
        $this->display('comment_mobile');
    }

	
	public function localCommentList($param)
	{	
	...
	public function getCommentList($app, $mod, $row_id, $page, $count)
    {
        ...
        $map = array('app' => $app, 'mod' => $mod, 'row_id' => $row_id,'siteid'=>SITEID ,'status' => 1);
